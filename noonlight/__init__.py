...
class NoonlightAlarm(object):
    ...
    def created_at(self):
        ...
        try:
            return datetime.strptime(
                self._json_data.get("created_at", "0001-01-01T00:00:00.00Z"),
                NOONLIGHT_DATETIME_FORMAT,
            )
        except:
            return datetime.min
    ...
    def locations(self):
        ...
        library.
        """
        locations_merged = self._json_data.get("locations", {}).get(
            "addresses", []
        ) + self._json_data.get("locations", {}).get("coordinates", [])
        for location in locations_merged:
            if (
                "created_at" in location
                and type(location["created_at"]) is not datetime
            ):
                try:
                    location["created_at"] = datetime.strptime(
                        location["created_at"], NOONLIGHT_DATETIME_FORMAT
                    )
                except:
                    pass
    ...
    async def cancel(self):
        ...
        :rtype: boolean
        """
        response = await self._client.update_alarm(
            id=self.id, body={"status": "CANCELED"}
        )
    ...
    async def _update_location_by_type(self, type, data):
        ...
        if type in ("coordinates", "address"):
            response = await self._client.update_alarm_location(
                id=self.id, body={type: data}
            )
            if type in response:
...
class NoonlightClient(object):
    ...
    def __init__(self, token, session=None, timeout=aiohttp.client.DEFAULT_TIMEOUT):
        """
    ...
    async def _post(self, path, data):
        async with self._session.post(path, json=data, headers=self._headers) as resp:
            if 200 <= resp.status < 300:
                return await resp.json()
            else:
                self.handle_error(resp.status, await resp.json())
    ...
    async def _put(self, path, data):
        async with self._session.put(path, json=data, headers=self._headers) as resp:
            if 200 <= resp.status < 300:
                return await resp.json()
            else:
                self.handle_error(resp.status, await resp.json())
